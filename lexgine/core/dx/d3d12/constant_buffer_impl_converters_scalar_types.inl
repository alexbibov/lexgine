...
public:
    ...
    {
        static_cast<bool_to_uint32<T>::type*>(p_raw_data_buffer)[0] = static_cast<bool_to_uint32<T>::type>(scalar_type);
    }

    //! resolves raw data into the corresponding HLSL type wrapper
    static T unconvert(void const* p_raw_data_buffer)
    {
        return static_cast<T>(static_cast<bool_to_uint32<T>::type const*>(p_raw_data_buffer)[0]);
    }
};
...
public:
    ...
    static void convert(void *p_raw_data_buffer, std::vector<bool> const& array_of_booleans)
    {
        ...
        for (size_t i = 0; i < array_of_booleans.size(); ++i)
            p_aux[i] = static_cast<uint32_t>(array_of_booleans[i]);
    }
