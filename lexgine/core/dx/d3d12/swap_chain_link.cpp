    ...
    // initialize depth buffers
    {
        ...
        auto descriptor =
            ResourceDescriptor::CreateTexture2D(back_buffer_dimensions.x, back_buffer_dimensions.y,
                queued_frames_count, m_depth_buffer_native_format);
        ...
        std::vector<ResourceState> depth_buffers_initial_states(queued_frames_count);
        HeapResourcePlacer placer{ *m_depth_buffer_heap };
        for (uint16_t i = 0U; i < queued_frames_count; ++i)
        {
            depth_buffers_initial_states[i] = ResourceState::enum_type::depth_read;
            m_depth_buffers.emplace_back(placer.addResource(
                depth_buffers_initial_states[i], m_depth_optimized_clear_value, descriptor));
            m_dsv_descriptors.emplace_back(m_depth_buffers.back(), DSVTextureArrayInfo{ 0, i, 1 });
        }

        m_depth_rendering_target.reset(new RenderingTargetDepth{ m_globals, m_depth_buffers,
            depth_buffers_initial_states, m_dsv_descriptors });
    }

    // initialize rendering targets
    {
        m_color_buffers.reserve(queued_frames_count);
        std::vector<ResourceState> color_buffers_initial_states(queued_frames_count);
        for (uint16_t i = 0U; i < queued_frames_count; ++i)
        {
            color_buffers_initial_states[i] = ResourceState::enum_type::common;
            m_color_buffers.emplace_back(m_linked_swap_chain.getBackBuffer(i));
            m_rtv_descriptors.emplace_back(m_color_buffers.back(), RTVTextureInfo{});
        }

        m_color_rendering_target.reset(new RenderingTargetColor{ m_globals, m_color_buffers, 
            color_buffers_initial_states, m_rtv_descriptors });
    }
}
