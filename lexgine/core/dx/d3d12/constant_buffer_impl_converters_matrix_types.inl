...
public:
    ...
    static void convert(void* p_raw_data_buffer, math::shader_matrix_type<bool, 2, 2> const& matrix2x2)
    {
        ...
        {
            p_aux[i] = static_cast<uint32_t>(p_matrix_data[i]);
        }
    }
...
public:
    ...
    static void convert(void *p_raw_data_buffer, math::shader_matrix_type<T, 3, 2> const& matrix3x2)
    {
        ...
        // First column
        p_aux[0] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[0]);
        p_aux[1] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[1]);
        p_aux[2] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[2]);

        // Second column
        p_aux[4] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[3]);
        p_aux[5] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[4]);
        p_aux[6] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[5]);
    }
...
public:
    ...
    static void convert(void *p_raw_data_buffer, math::shader_matrix_type<T, 3, 3> const& matrix3x3)
    {
        ...
        // Third column
        p_aux[8] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[6]);
        p_aux[9] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[7]);
        p_aux[10] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[8]);
    }
...
public:
    ...
    static void convert(void *p_raw_data_buffer, math::shader_matrix_type<T, 3, 4> const& matrix3x4)
    {
        ...
        // Fourth column
        p_aux[12] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[9]);
        p_aux[13] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[10]);
        p_aux[14] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[11]);
    }
...
public:
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<T, 2, 2>> const& matrix2x2_array)
    {
        ...
        for (size_t i = 0; i < matrix2x2_array.size(); ++i)
            memcpy(p_aux + 4 * i, matrix2x2_array[i].getRawData(), sizeof(T) * 4);
    }
    ...
    static std::vector<math::shader_matrix_type<T, 2, 2>> unconvert(void const* p_raw_data_buffer, size_t size_of_raw_data_in_bytes)
    {
        ...
        for (size_t i = 0; i < static_cast<size_t>(num_elements); ++i)
            memcpy(rv[i].getRawData(), p_aux + 4 * i, sizeof(T) * 4);
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<bool, 2, 2>> const& matrix2x2_array)
    {
        ...
        for (size_t i = 0; i < matrix2x2_array.size(); ++i)
        {
            ...
            for (uint8_t j = 0; j < 4U; ++j)
                p_aux[i * 4 + j] = static_cast<uint32_t>(p_matrix_data[j]);
        }
    }
...
public:
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<T, 2, 3>> const& matrix2x3_array)
    {
        ...
        for (size_t i = 0; i < matrix2x3_array.size(); ++i)
            memcpy(p_aux + 6 * i, matrix2x3_array[i].getRawData(), sizeof(T) * 6);
    }
    ...
    static std::vector<math::shader_matrix_type<T, 2, 3>> unconvert(void const* p_raw_data_buffer, size_t size_of_raw_data_in_bytes)
    {
        ...
        for (size_t i = 0; i < static_cast<size_t>(num_elements); ++i)
            memcpy(rv[i].getRawData(), p_aux + 6 * i, sizeof(T) * 6);
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<bool, 2, 3>> const& matrix2x3_array)
    {
        ...
        for (size_t i = 0; i < matrix2x3_array.size(); ++i)
        {
            ...
            for (uint8_t j = 0; j < 6U; ++j)
                p_aux[i * 6 + j] = static_cast<uint32_t>(p_matrix_data[j]);
        }
    }
...
public:
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<T, 2, 4>> const& matrix2x4_array)
    {
        ...
        for (size_t i = 0; i < matrix2x4_array.size(); ++i)
            memcpy(p_aux + 8 * i, matrix2x4_array[i].getRawData(), sizeof(T) * 8);
    }
    ...
    static std::vector<math::shader_matrix_type<T, 2, 4>> unconvert(void const* p_raw_data_buffer, size_t size_of_raw_data_in_bytes)
    {
        ...
        for (size_t i = 0; i < static_cast<size_t>(num_elements); ++i)
            memcpy(rv[i].getRawData(), p_aux + 8 * i, sizeof(T) * 8);
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<bool, 2, 4>> const& matrix2x4_array)
    {
        ...
        for (size_t i = 0; i < matrix2x4_array.size(); ++i)
        {
            ...
            for (uint8_t j = 0; j < 8U; ++j)
                p_aux[i * 8 + j] = static_cast<uint32_t>(p_matrix_data[j]);
        }
    }
...
public:
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<T, 3, 2>> const& matrix3x2_array)
    {
        ...
        for (size_t i = 0; i < matrix3x2_array.size(); ++i)
        {
            ...
            // First column
            p_aux[8 * i] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[0]);
            p_aux[8 * i + 1] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[1]);
            p_aux[8 * i + 2] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[2]);

            // Second column
            p_aux[8 * i + 4] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[3]);
            p_aux[8 * i + 5] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[4]);
            p_aux[8 * i + 6] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[5]);
        }
    }
...
public:
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<T, 3, 3>> const& matrix3x3_array)
    {
        ...
        for (size_t i = 0; i < matrix3x3_array.size(); ++i)
        {
            ...
            // First column
            p_aux[12 * i] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[0]);
            p_aux[12 * i + 1] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[1]);
            p_aux[12 * i + 2] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[2]);

            // Second column
            p_aux[12 * i + 4] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[3]);
            p_aux[12 * i + 5] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[4]);
            p_aux[12 * i + 6] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[5]);

            // Third column
            p_aux[12 * i + 8] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[6]);
            p_aux[12 * i + 9] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[7]);
            p_aux[12 * i + 10] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[8]);
        }
    }
...
public:
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<T, 3, 4>> const& matrix3x4_array)
    {
        ...
        for (size_t i = 0; i < matrix3x4_array.size(); ++i)
        {
            ...
            // First column
            p_aux[16 * i] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[0]);
            p_aux[16 * i + 1] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[1]);
            p_aux[16 * i + 2] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[2]);

            // Second column
            p_aux[16 * i + 4] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[3]);
            p_aux[16 * i + 5] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[4]);
            p_aux[16 * i + 6] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[5]);

            // Third column
            p_aux[16 * i + 8] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[6]);
            p_aux[16 * i + 9] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[7]);
            p_aux[16 * i + 10] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[8]);

            // Fourth column
            p_aux[16 * i + 12] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[9]);
            p_aux[16 * i + 13] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[10]);
            p_aux[16 * i + 14] = static_cast<bool_to_uint32<T>::type>(p_matrix_data[11]);
        }
    }
...
public:
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<T, 4, 2>> const& matrix4x2_array)
    {
        ...
        {
            memcpy(p_aux + 8 * i, matrix4x2_array[i].getRawData(), sizeof(T) * 8);
        }
    }
...
template<> class constant_buffer_hlsl_type_raw_data_converter<std::vector<math::shader_matrix_type<bool, 4, 2>>>
{
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<bool, 4, 2>> const& matrix4x2_array)
    {
        ...
        for (size_t i = 0; i < matrix4x2_array.size(); ++i)
        {
            ...
            for (uint8_t j = 0; j < 8U; ++j)
                p_aux[8 * i + j] = static_cast<uint32_t>(p_matrix_data[j]);
        }
    }
...
public:
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<T, 4, 3>> const& matrix4x3_array)
    {
        ...
        {
            memcpy(p_aux + 12 * i, matrix4x3_array[i].getRawData(), sizeof(T) * 12);
        }
    }
...
template<> class constant_buffer_hlsl_type_raw_data_converter<std::vector<math::shader_matrix_type<bool, 4, 3>>>
{
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<bool, 4, 3>> const& matrix4x3_array)
    {
        ...
        for (size_t i = 0; i < matrix4x3_array.size(); ++i)
        {
            ...
            for (uint8_t j = 0; j < 12U; ++j)
                p_aux[12 * i + j] = static_cast<uint32_t>(p_matrix_data[j]);
        }
    }
    ...
    static std::vector<math::shader_matrix_type<bool, 4, 3>> unconvert(void const* p_raw_data_buffer, size_t size_of_raw_data_in_bytes)
    {
        ...
        for (size_t i = 0; i < static_cast<size_t>(num_elements); ++i)
        {
            bool* p_matrix_data = rv[i].getRawData();
            for (uint8_t j = 0; j < 12U; ++j)
                p_matrix_data[j] = p_aux[12 * i + j] != 0;
        }
    }
};
...
public:
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<T, 4, 4>> const& matrix4x4_array)
    {
        ...
        {
            memcpy(p_aux + 16 * i, matrix4x4_array[i].getRawData(), sizeof(T) * 16);
        }
    }
...
template<> class constant_buffer_hlsl_type_raw_data_converter<std::vector<math::shader_matrix_type<bool, 4, 4>>>
{
    ...
    static void convert(void *p_raw_data_buffer, std::vector<math::shader_matrix_type<bool, 4, 4>> const& matrix4x4_array)
    {
        ...
        for (size_t i = 0; i < matrix4x4_array.size(); ++i)
        {
            ...
            for (uint8_t j = 0; j < 16U; ++j)
                p_aux[16 * i + j] = static_cast<uint32_t>(p_matrix_data[j]);
        }
    }
    ...
    static std::vector<math::shader_matrix_type<bool, 4, 4>> unconvert(void const* p_raw_data_buffer, size_t size_of_raw_data_in_bytes)
    {
        ...
        for (size_t i = 0; i < static_cast<size_t>(num_elements); ++i)
        {
            bool* p_matrix_data = rv[i].getRawData();
            for (uint8_t j = 0; j < 16U; ++j)
                p_matrix_data[j] = p_aux[16 * i + j] != 0;
        }
    }
};