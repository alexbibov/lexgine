...
public:
    static constexpr size_t serialized_size = sizeof(cache_entry_type) +
        (std::max)(RootSignatureBlobCache::Key::serialized_size,
        (std::max)(HLSLCompilationTaskCache::Key::serialized_size,
            PSOCompilationTaskCache::Key::serialized_size));

    CombinedCacheKey(RootSignatureBlobCache::Key const& key);
    ...
    operator RootSignatureBlobCache::Key const&() const;
...
private:
    ...
    union maintained_key{
        RootSignatureBlobCache::Key rs;
        PSOCompilationTaskCache::Key pso;
        HLSLCompilationTaskCache::Key hlsl;

        maintained_key(RootSignatureBlobCache::Key const& key);
