...
Submesh::Submesh(Scene const& owner)
    : m_owner{ owner }
    , m_scene_memory{ m_owner.getSceneMemory() }
    , m_instance_count{ 1 }
{
...
)
{
    ...
    m_vb_binding.setVertexBufferView(
        ...
        m_scene_memory.scene_memory_buffer->getGPUResource(),
...
void Submesh::setBaseMaterial(uint32_t material_id)
{
    m_base_material_id = material_id;
    Material const& material = m_owner.getSceneMaterial(material_id);
    m_object_parameters_data_mapper = std::make_unique<core::dx::d3d12::ConstantBufferDataMapper>(
        material.getStaticState().getObjectParametersUniformBufferReflection()
    );
}
