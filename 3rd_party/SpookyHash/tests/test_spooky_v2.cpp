...
TEST(SpookyHash_Tests, DoTimingBig)
{
#define NUMBUF (1<<10)
#define BUFSIZE (1<<20)
    int seed = static_cast<int>(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count());
    ...
    {
        spooky_hash_v2::SpookyHash::Hash128(buf[i], BUFSIZE, &hash1, &hash2);
    }
    ...
    {
        spooky_hash_v2::SpookyHash::Hash128(buf[0], 1024, &hash1, &hash2);
    }
    z = GetTickCount();
...
TEST(SpookyHash_Tests, DoTimingSmall)
{
#define BUFSIZE (1<<14)
#define NUMITER 10000000
    ...
    for (int i = 1; i <= BUFSIZE; i <<= 1)
    {
        ...
        {
            spooky_hash_v2::SpookyHash::Hash128((char*)buf, i, &hash1, &hash2);
        }
        uint64_t z = GetTickCount();
        ...
        for (int i = 0; i < h * 8; ++i)
        {
            ...
            for (int j = 0; j <= i; ++j)
            {
                ...
                for (k = 0; k < TRIES; ++k)
                {
                    ...
                    spooky_hash_v2::SpookyHash::Hash128(buf1, h, &measure[0][0], &measure[0][1]);
                    spooky_hash_v2::SpookyHash::Hash128(buf2, h, &measure[1][0], &measure[1][1]);
                    for (int l = 0; l < 2; ++l) {
                ...
                EXPECT_LT(k, TRIES) << "failed " << h << " " << i << " " << j << std::endl;
                if (k > maxk)
                {
                    maxk = k;
                }
...
#define BUFSIZE (512)
    ...
    {
        buf[i] = i + 128;
        saw[i] = spooky_hash_v2::SpookyHash::Hash32(buf, i, 0);
        EXPECT_EQ(saw[i], expected[i]) << i << ": saw " << std::hex << saw[i] << ", expected " << expected[i] << std::endl;
    }
#undef BUFSIZE
}


TEST(SpookyHash_Tests, Alignment)
{
#define BUFSIZE 1024
    ...
    for (int i = 0; i < BUFSIZE - 16; ++i)
    {
        for (int j = 0; j < 8; ++j)
        {
            ...
            buf[j + i + 1] = (char)i + j;
            hash[j] = spooky_hash_v2::SpookyHash::Hash64((const void*)(buf + j + 1), i, 0);
        }
        for (int j = 1; j < 8; ++j)
        {
            EXPECT_EQ(hash[0], hash[j]) << "alignment problems: " << i << " " << j << std::endl;
        }
    }
#undef BUFSIZE
}
...
#define BUFSIZE 1024
    ...
    for (int i = 0; i < BUFSIZE; ++i)
    {
        ...
        spooky_hash_v2::SpookyHash state;

        // all as one call
        a = seed1;
        b = seed2;
        spooky_hash_v2::SpookyHash::Hash128(buf, i, &a, &b);
        ...
        EXPECT_EQ(a, c) << "wrong a " << i << ": " << std::hex << a << " " << c << std::endl;
        EXPECT_EQ(b, d) << "wrong b " << i << ": " << std::hex << b << " " << d << std::endl;
        ...
        for (int j = 0; j < i; ++j)
        {
            ...
            EXPECT_EQ(a, c) << "wrong a " << j << " " << i << ": " << std::hex << a << " " << c << std::endl;
            EXPECT_EQ(b, d) << "wrong b " << j << " " << i << ": " << std::hex << b << " " << d << std::endl;
        }
    }
#undef BUFSIZE
}