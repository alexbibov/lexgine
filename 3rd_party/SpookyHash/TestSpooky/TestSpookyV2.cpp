...
#define BUFSIZE (512)
        void TestResults()
        {
            ...
            for (int i = 0; i < BUFSIZE; ++i)
            {
                ...
                wchar_t message_buffer[80];
                std::swprintf(message_buffer, 80, L"%3d: saw 0x%.8lx, expected 0x%.8lx\n", i, saw[i], static_cast<unsigned long>(expected[i]));

                Assert::IsFalse(saw[i] != expected[i], message_buffer);
...
#define BUFSIZE 1024
        void TestAlignment()
        {
            ...
            for (int i = 0; i < BUFSIZE - 16; ++i)
            {
                ...
                {
                    wchar_t message_buffer[80];
                    std::swprintf(message_buffer, 80, L"alignment problems: %d %d\n", i, j);
                    Assert::IsFalse(hash[0] != hash[j], message_buffer);
            ...
            for (int h = 0; h < BUFSIZE; ++h)
            {
                ...
                for (int i = 0; i < h * 8; ++i)
                {
                    ...
                    for (int j = 0; j <= i; ++j)
                    {
                        ...
                        wchar_t message_buffer[80];
                        std::swprintf(message_buffer, 80, L"failed %d %d %d\n", h, i, j);
                        Assert::IsFalse(k == TRIES, message_buffer);
                        /*if (k == TRIES)
                        {
                            printf("failed %d %d %d\n", h, i, j);
                        }
...
#define BUFSIZE 1024
        void TestPieces()
        {
            ...
            for (int i = 0; i < BUFSIZE; ++i)
            {
                ...
                wchar_t message_buffer[80];
                std::swprintf(message_buffer, 80, L"wrong a %d: %.16llx %.16llx\n", i, a, c);
                Assert::IsFalse(a != c, message_buffer);

                std::swprintf(message_buffer, 80, L"wrong b %d: %.16llx %.16llx\n", i, b, d);
                Assert::IsFalse(b != d, message_buffer);
                /*if (a != c)
                {
                ...
                for (int j = 0; j < i; ++j)
                {
                    ...
                    std::swprintf(message_buffer, 80, L"wrong a %d %d: %.16llx %.16llx\n", j, i, a, c);
                    Assert::IsFalse(a != c, message_buffer);
                    std::swprintf(message_buffer, 80, L"wrong b %d %d: %.16llx %.16llx\n", j, i, b, d);
                    Assert::IsFalse(b != d, message_buffer);
                    /*if (a != c)
                    {
