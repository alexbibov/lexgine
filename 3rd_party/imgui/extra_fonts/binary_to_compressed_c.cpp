...
#define STB__TRY(t,p)  /* avoid retrying a match we already tried */ \
    ...
    if (stb__nc(m,static_cast<int>(q-(t))))                                \
    best = m, dist = static_cast<int>(q - (t))
        ...
        if (best < 3) { // fast path literals
            ...
            outliterals(lit_start, static_cast<int>(q-lit_start)); lit_start = (q += best);
            stb_out(0x80 + best-1);
            stb_out(dist-1);
    ...
    *pending_literals = static_cast<int>(q - lit_start);
    ...
    stb__running_adler = stb_adler32(stb__running_adler, start, static_cast<int>(q - start));
